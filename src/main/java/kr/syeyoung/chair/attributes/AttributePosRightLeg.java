...
public class AttributePosRightLeg extends ChairEntityAttribute<SerializedEulerAngle> {
    public AttributePosRightLeg() {
        this.setAttributeValue(new SerializedEulerAngle(0,0,0));
    }
    @Override
    public String getAttributeName() {
        return "pos_right_leg";
    }
    ...
    public boolean applyAttribute(Entity e) {
        if (e instanceof ArmorStand) {
            ((ArmorStand) e).setRightLegPose(getAttributeValue());
            return true;
        }
        return false;
    }
    ...
    public SerializedEulerAngle extractAttribute(Entity e) {
        if (e instanceof ArmorStand) {
            setAttributeValue(new SerializedEulerAngle(((ArmorStand) e).getRightLegPose()));
    ...
    public ChairEntityAttribute<SerializedEulerAngle> clone() {
        return new AttributePosRightLeg().setAttributeValue(getAttributeValue());
    }
}
